...
ScrollTrigger.create({
  trigger: ".svg-box",
  start: "top center",
  // .svg-box가 뷰포트 아래쪽에 닿을 때
  end: "bottom top",
  // .svg-box가 뷰포트 위쪽으로 사라질 때
...
ScrollTrigger.create({
  ...
  start: "top top",
  // .page2의 상단이 뷰포트 상단에 도달했을 때 시작
  end: () => `+=${getWorkListWidth()}`,
  // 6000px 만큼 스크롤할 동안 애니메이션 진행
  pin: true,
  // .page2를 고정
...
gsap.to(".content-wrap", {
  x: () => -getWorkListWidth(),
  // .content-wrap의 이동 거리를 동적으로 계산
  ease: "none",
  scrollTrigger: {
    trigger: ".sec-portfolio-work",
    start: "top top",
    end: () => `+=${getWorkListWidth()}`,
    // .work-list의 너비만큼 스크롤 구간 설정
    scrub: true
  }
});
